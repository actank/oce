...
UserBasicDataPtr UserBasicDataI::getData()
{
	IceUtil::Mutex::Lock lock(m_mutex);
...
{
	UserBasicDataPtr data = UserBasicDataPtr::dynamicCast(obj);
	id = data->id ;
	univ = data->univ ;
	name = data->name ;
	status = data->status ;
...
UserBornDataPtr UserBornDataI::getOldData()
{
	IceUtil::Mutex::Lock lock(m_mutex);
	
	UserBornDataPtr result = new UserBornData;
...
void UserStageDataI::parse(const UserBasicDataIPtr& obj)
{
	id = obj->id ;
	univ = obj->univ ;
	stage = obj->stage ;
...
void UserDoingDataI::parse(const UserBasicDataIPtr& obj)
{
	id = obj->id ;
	doing= obj->doing;
	doingTime= obj->doingTime;
}
