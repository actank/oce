...
Ice::Int IndexList::getCountByType(const MyUtil::IntSeq& types) {
	...
	//  MCE_INFO("getCount from database : uid=" << _uid);
...
bool NotifyIndexManagerI::_getContentsByIds(int uid,const MyUtil::LongSeq & ids,
	...
	try {
		...
		MCE_WARN("NotifyContentAdapter::getNotifyDict() error:" << e);
		return false;
	}
...
void NotifyIndexManagerI::Evict(){
	...
	MCE_INFO("NotifyIndexManagerI::Evict. cnt:"<<cnt << " miss:" << miss
...
MyUtil::Int2IntMap NotifyIndexManagerI::GetUnreadCountByBigtypes(int userid, const MyUtil::IntSeq& types, const Ice::Current&){
	MCE_DEBUG("NotifyIndexManagerI::GetUnreadCountByBigtypes--> userid:" << userid << " types.size():" << types.size());
...
void NotifyIndexManagerI::RemoveRMessageById(int uid, long notifyId, int type, int bigtype, const Ice::Current&){
	...
	UserMessagePoolPtr p = GetRMessagePool(uid);
	if(p){
		MCE_INFO("NotifyIndexManagerI::RemoveRMessageById-->userid:" << uid << " notifyid:" << notifyId << " bigtype:" << bigtype);
...
void NotifyIndexManagerI::RemoveRMessageByIds(int uid, const MyUtil::LongSeq& notifyIds, int type, int bigtype, const Ice::Current&){
	...
	UserMessagePoolPtr p = GetRMessagePool(uid);
	if(p){
		MCE_DEBUG("NotifyIndexManagerI::RemoveRMessageByIds --> ~~~~~~~~~~~~~~~~~~~~~~~ uid << " << uid);
