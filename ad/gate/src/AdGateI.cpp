...
bool AdGetRequest::response(){
  ...
  if (true) {
    try {
      ...
      z2c = AdEngineUAdapter::instance().GetAdsByZoneInfo(serialized, append_para.str(), rotate_slot, request_zones, _login_uid);
  ...
  if(z2c.empty()) {
    ...
    for(AdResultMap::const_iterator it = ads.begin(); it != ads.end(); ++it) {
      MCE_INFO("AdEngineUAdapter/AdTwAdapter::GetAds add default ad : " << it->first << '/' << it->second.size());
    }
    translate(ads, z2c);
  } else {
    for(AdZone2Creatives::const_iterator it = z2c.begin(); it != z2c.end(); ++it) {
      MCE_INFO("AdEngineUAdapter/AdTwAdapter::GetAds return : " << it->first << '/' << it->second.size());
    }
  }
