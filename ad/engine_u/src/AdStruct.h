...
public:
  ...
  vector<AdGroupPtr> GetAllDefaultGroups(){
...
public:
  ...
  //  return max_price_;
  //}
  double max_price(Ice::Long zone_id) {
  ...
  double calculate_cost(Ice::Long zone_id) {
    ...
    double max_price = this->max_price(zone_id);
    double cost = ((cost_discount + rand() % rand_seed / 10000.0) * (max_price));
    if(IsCpc()){	//cpc
    	if (cost < EngineConfig::instance().sys_min_price()) {
    	...
    	if((widget_id() == 31) && (cost < 200)) {
    		cost = 200;
    	}
    	if(group_id_ == 10000345660004) {
    		MCE_DEBUG("calcualte cost:gid:" << group_id_ << ",max_price:" << max_price << ",cost:" << cost <<",zone:" << zone_id);
    	}
    } else if(IsCpm()){	//cpm
   		cost /= 1000;
    }
   /*if(group_id_==10000351380002)
    {
                      MCE_INFO("calcualte cost:gid:" << group_id_ << ",max_price:" << max_price << ",cost:" << cost <<",zone:" << zone_id);
    }*/
    return cost;
  }
  int rotation_number() {
...
private:
  ...
  map<Ice::Long,double> zone_price_;
