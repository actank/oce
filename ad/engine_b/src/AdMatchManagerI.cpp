...
void MyUtil::initialize() {
  ...
  xce::serverstate::ServerStateSubscriber::instance().initialize("ControllerAdEngineB",&AdMatchManagerI::instance(),mod,interval, new AdChannel());
  ...
  if (!para.ubc_.empty()) {
    ...
    if (group_ptr) {
      ...
      clicklog << para.creative_id_ << "," << date.toDateTime() << "," << group_ptr->bid_unit_id()<< "," << para.cost_ << ","
          ...
          << pv_date.toDateTime() <<"," << uid << ",2,1,1," << ip << ",,," << group_ptr->max_price() << ",1,lm," << EngineConfig::instance().sys_min_price() << ','
