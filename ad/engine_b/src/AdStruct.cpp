...
void AdZone::random_rotate() {
  ...
  if(member_type() <= 1 && rotate_list_.size() > 0) {
    int real_size = 0;
    for(; real_size < rotate_list_.size(); ++real_size) {
...
Ice::Long AdGroup::GetCreative(int uid, int rotate_index) {
  ...
  if(NielsonFilter::instance().Filter(uid, group_id(), ad_type) && !sa_creative_ids_.empty() && !saa_creative_ids_.empty()) {
    ...
    MCE_DEBUG("AdGroup::GetCreative() --> uid = " << uid << ", group_id = " << group_id() << ", normal creative should display.");
...
void UserBindPool::BindUser(int user_id, Ice::Long creative_id,
  ...
  try{
    AdEngineBAdapter::instance().NotifyBindUser(user_id, creative_id, zone_id);
  }
  catch(Ice::Exception& e){
    MCE_WARN("AdEngineBAdapter::BindUser --> call AdEngineBAdapter.BindUser ERR  line:"<<__LINE__<<" err:"<<e);
  }
  catch(std::exception& e){
    MCE_WARN("AdEngineBAdapter::BindUser --> call AdEngineBAdapter.BindUser ERR  line:"<<__LINE__<<" err:"<<e.what());
  }
}
void UserBindPool::UnbindUser(int user_id, Ice::Long zone_id) {
  ...
  mysqlpp::StoreQueryResult res = QueryRunner(DB_INSTANCE, CDbWServer, "ad_user_bind").store(sql);
  try{
    AdEngineBAdapter::instance().NotifyUnbindUser(user_id, zone_id);
  }
  catch(Ice::Exception& e){
    MCE_WARN("AdEngineBAdapter::BindUser --> call AdEngineBAdapter.UnbindUser ERR  line:"<<__LINE__<<" err:"<<e);
  }
  catch(std::exception& e){
    MCE_WARN("AdEngineBAdapter::BindUser --> call AdEngineBAdapter.UnbindUser ERR  line:"<<__LINE__<<" err:"<<e.what());
  }
}
