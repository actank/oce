...
void FeedAssistantI::dispatch(const FeedSeedPtr& seed, const Ice::Current&) {
  if (seed) {
    ...
    md.feedid = seed->feed;
    md.type = seed->type;
    md.userid = seed->actor;
    md.time = seed->time;
    md.weight = seed->baseWeight;
    md.mergeid = seed->newsMerge;
    FeedItemCacheAdapter::instance().put(md);*/
  }
}
...
//  }
  ...
  //FeedInteractionMemcClient::instance().IncRelplyCount(feedid,1);
