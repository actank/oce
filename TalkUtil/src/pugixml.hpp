...
const size_t memory_block_size = 8192;
...
const unsigned int parse_default = parse_cdata | parse_escapes
		| parse_wconv_attribute | parse_eol | parse_ws_pcdata;
...
public:
	...
	/**
	 * Get attribute value.
	 * this funcation is only used by self, the return value is not decode
...
public:
	...
	/**
	 *
	 *  return decode node value (only for pcdata)
	 */
	std::string value() const;
	 ...
	 */
	std::string child_value(const std::string& name) const;
	 ...
	 */
	std::string child_value_w(const std::string& name)const;
public:
	/**
 ...
 */
class xml_document: public xml_node, public Ice::Object
{
private:
	char* _buffer;
...
/**
 *  add IceUtil::Shared smart point
 *
 */
typedef IceUtil::Handle<xml_document> xml_document_ptr;
...
template<typename OutputIterator> void xml_node::all_elements_by_name(
		const char* name, OutputIterator it) const {
	if (empty())
		return;
	...
	for (xml_node node = first_child(); node; node = node.next_sibling()) {
		if (node.type() == node_element) {
			...
			if (node.first_child())
				node.all_elements_by_name(name, it);
		}
	}
}

template<typename OutputIterator> void xml_node::all_elements_by_name_w(
		const char* name, OutputIterator it) const {
	if (empty())
		return;
	...
	for (xml_node node = first_child(); node; node = node.next_sibling()) {
		if (node.type() == node_element) {
			...
			if (node.first_child())
				node.all_elements_by_name_w(name, it);
		}
	}
}

template<typename Predicate> inline xml_attribute xml_node::find_attribute(
		Predicate pred) const {
	if (!empty())
		for (xml_attribute attrib = first_attribute(); attrib; attrib
				= attrib.next_attribute())
			if (pred(attrib))
				return attrib;
