...
#define UTF8_INVALID2(p) \
  ((unsigned char)(*p) < 0xC2 || ((unsigned char)(p)[1] & 0x80) == 0 || ((unsigned char)(p)[1] & 0xC0) == 0xC0)

#define UTF8_INVALID3(p) \
  (((unsigned char)(p)[2] & 0x80) == 0 \
  || \
  ((unsigned char)(*p) == 0xEF && (unsigned char)(p)[1] == 0xBF \
  ? \
   (unsigned char)(p)[2] > 0xBD \
  : \
  ((unsigned char)(p)[2] & 0xC0) == 0xC0) \
  || \
  ((unsigned char)(*p) == 0xE0 \
  ? \
  (unsigned char)(p)[1] < 0xA0 || ((unsigned char)(p)[1] & 0xC0) == 0xC0 \
  : \
  ((unsigned char)(p)[1] & 0x80) == 0 \
  || \
  ((unsigned char)(*p) == 0xED ? (unsigned char)(p)[1] > 0x9F : ((unsigned char)(p)[1] & 0xC0) == 0xC0)))

#define UTF8_INVALID4(p) \
  (((unsigned char)(p)[3] & 0x80) == 0 || ((unsigned char)(p)[3] & 0xC0) == 0xC0 \
  || \
  ((unsigned char)(p)[2] & 0x80) == 0 || ((unsigned char)(p)[2] & 0xC0) == 0xC0 \
  || \
  ((unsigned char)(*p) == 0xF0 \
  ? \
  (unsigned char)(p)[1] < 0x90 || ((unsigned char)(p)[1] & 0xC0) == 0xC0 \
  : \
  ((unsigned char)(p)[1] & 0x80) == 0 \
  || \
  ((unsigned char)(*p) == 0xF4 ? (unsigned char)(p)[1] > 0x8F : ((unsigned char)(p)[1] & 0xC0) == 0xC0)))
...
class xml_allocator {
public:
	xml_allocator(xml_memory_block* root) :
		_root(root) {
	}
...
struct xml_attribute_struct {
	...
	xml_attribute_struct() :
		name_insitu(true), value_insitu(true), document_order(0), name(0),
				value(0), prev_attribute(0), next_attribute(0) {
	}
...
struct xml_node_struct {
	...
	xml_node_struct(xml_node_type type = node_element) :
		type(type), name_insitu(true), value_insitu(true), document_order(0),
				parent(0), name(0), value(0), first_child(0), last_child(0),
				prev_sibling(0), next_sibling(0), first_attribute(0),
				last_attribute(0) {
	}
	...
	void destroy() {
		...
		for (xml_attribute_struct* attr = first_attribute; attr; attr
				= attr->next_attribute)
			attr->destroy();

		for (xml_node_struct* node = first_child; node; node
				= node->next_sibling)
			node->destroy();
	}

	xml_node_struct* append_node(xml_allocator& alloc,
	...
	xml_attribute_struct* append_attribute(xml_allocator& alloc) {
		...
		if (last_attribute) {
			...
			first_attribute = last_attribute = a;
...
struct xml_document_struct : public xml_node_struct {
	xml_document_struct() :
		xml_node_struct(node_document), allocator(0) {
	}
...
const unsigned char UTF8_FIRST_BYTE_MARK[7] = { 0x00, 0x00, 0xC0, 0xE0, 0xF0,
		0xF8, 0xFC };
...
const unsigned char chartype_table[256] = { 55, 0, 0, 0, 0, 0, 0, 0, 0,
		12,
		12,
		0,
		0,
		...
		192,
		192, // 96-111
		192, 192, 192, 192, 192, 192, 192, 192, 192, 192, 192, 0,
		0,
		0,
		0,
		0, // 112-127
...
template<bool _1, bool _2, bool _3, bool _4> const bool opt4_to_type<_1, _2,
		_3, _4>::o1 = _1;
template<bool _1, bool _2, bool _3, bool _4> const bool opt4_to_type<_1, _2,
		_3, _4>::o2 = _2;
template<bool _1, bool _2, bool _3, bool _4> const bool opt4_to_type<_1, _2,
...
#ifndef PUGIXML_NO_STL
template<typename opt2> void text_output_escaped(std::ostream& os,
		const char* s, opt2) {
	...
	while (*s) {
		...
		if (prev != s)
			os.write(prev, static_cast<std::streamsize>(s - prev));
		...
		default: // s is not a usual symbol
		{
			...
			//add by zta , ch  > 20 not encode
			unsigned char ch = *s++;
			if (ch <= 20 && ch != 10 && ch != 13 && ch != 9) {
				//int i = ch;
				//os << "&#" << i << ";";
				os << "?";
			} else
...
template<typename opt2> void text_output_escaped(std::string& os,
		const char* s, opt2) {
	const bool attribute = opt2::o1;
	// const bool utf8 = opt2::o2;
...
char* strconv_pcdata(char* s, unsigned int optmask) {
	...
	{
	case 0:
		return strconv_pcdata_t(s, opt2_to_type<0, 0>());
	case 1:
		return strconv_pcdata_t(s, opt2_to_type<0, 1>());
	case 2:
		return strconv_pcdata_t(s, opt2_to_type<1, 0>());
	case 3:
		return strconv_pcdata_t(s, opt2_to_type<1, 1>());
	default:
...
template<typename opt2> char* strconv_pcdata_t_set_value(char* s, opt2) {
	const bool opt_eol = opt2::o1;
	const bool opt_escape = opt2::o2;
...
char* strconv_pcdata_set_value(char* s, unsigned int optmask) {
	STATIC_ASSERT(parse_escapes == 0x10 && parse_eol == 0x20);

	switch ((optmask >> 4) & 3) // get bitmask for flags (eol escapes)
	{
	case 0:
		return strconv_pcdata_t_set_value(s, opt2_to_type<0, 0>());
	case 1:
		return strconv_pcdata_t_set_value(s, opt2_to_type<0, 1>());
	case 2:
		return strconv_pcdata_t_set_value(s, opt2_to_type<1, 0>());
	case 3:
		return strconv_pcdata_t_set_value(s, opt2_to_type<1, 1>());
	default:
		return 0; // should not get here
	}
}
...
template<typename opt4> char* strconv_attribute_t(char* s, char end_quote, opt4) {
	...
	while (true) {
		while (!is_chartype(*s, (opt_wnorm || opt_wconv) ? ct_parse_attr_ws
				: ct_parse_attr))
			++s;
...
char* strconv_attribute(char* s, char end_quote, unsigned int optmask) {
	...
	{
	case 0:
		return strconv_attribute_t(s, end_quote, opt4_to_type<0, 0, 0, 0>());
	case 1:
		return strconv_attribute_t(s, end_quote, opt4_to_type<0, 0, 0, 1>());
	case 2:
		return strconv_attribute_t(s, end_quote, opt4_to_type<0, 0, 1, 0>());
	case 3:
		return strconv_attribute_t(s, end_quote, opt4_to_type<0, 0, 1, 1>());
	case 4:
		return strconv_attribute_t(s, end_quote, opt4_to_type<0, 1, 0, 0>());
	case 5:
		return strconv_attribute_t(s, end_quote, opt4_to_type<0, 1, 0, 1>());
	case 6:
		return strconv_attribute_t(s, end_quote, opt4_to_type<0, 1, 1, 0>());
	case 7:
		return strconv_attribute_t(s, end_quote, opt4_to_type<0, 1, 1, 1>());
	case 8:
		return strconv_attribute_t(s, end_quote, opt4_to_type<1, 0, 0, 0>());
	case 9:
		return strconv_attribute_t(s, end_quote, opt4_to_type<1, 0, 0, 1>());
	case 10:
		return strconv_attribute_t(s, end_quote, opt4_to_type<1, 0, 1, 0>());
	case 11:
		return strconv_attribute_t(s, end_quote, opt4_to_type<1, 0, 1, 1>());
	case 12:
		return strconv_attribute_t(s, end_quote, opt4_to_type<1, 1, 0, 0>());
	case 13:
		return strconv_attribute_t(s, end_quote, opt4_to_type<1, 1, 0, 1>());
	case 14:
		return strconv_attribute_t(s, end_quote, opt4_to_type<1, 1, 1, 0>());
	case 15:
		return strconv_attribute_t(s, end_quote, opt4_to_type<1, 1, 1, 1>());
	default:
		return 0; // should not get here
	}
}
...
#define CHECK_ERROR()		{ if (*s == 0) return false; }
	...
	xml_parser(xml_allocator& alloc) :
		alloc(alloc) {
	}

	bool parse(char* s, xml_node_struct* xmldoc,
			unsigned int optmsk = parse_default) {
		if (!s || !xmldoc)
			return false;

		// UTF-8 BOM
		if ((unsigned char)*s == 0xEF && (unsigned char)*(s+1) == 0xBB
			...
			if (*s == '<') {
				...
				LOC_TAG: if (*s == '?') // '<?...'
				{
					...
					if (!is_chartype(*s, ct_start_symbol)) // bad PI
						...
						// stricmp / strcasecmp is not portable
						else if ((mark[0] == 'x' || mark[0] == 'X') && (mark[1]
								== 'm' || mark[1] == 'M') && (mark[2] == 'l'
								|| mark[2] == 'L') && mark[3] == 0) {
						...
						while (*s) {
							...
							if (is_chartype(*s, ct_start_symbol)) // <... #...
							{
								xml_attribute_struct* a =
										cursor->append_attribute(alloc); // Make space for this attribute.
		...
		if (impl::strcmpwild(*src, *dst)) {
			do {
				...
				while (**src != **dst && **src != '[' && **dst != 0)
					++(*dst);
			} while ((**dst != 0) ? impl::strcmpwild(*src, *dst) : 0
					!= (find=0));
		}
		if (**dst == 0 && **src == 0)
			find = 1;
...
const char* xml_attribute::_value() const
{
	return (!empty() && _attr->value) ? _attr->value : "";
}
//add by zta
std::string xml_attribute::value() const
{
	//return (!empty() && _attr->value) ? _attr->value : "";
	if(!empty() && _attr->value) {
		std::ostringstream os;
		text_output_escaped(os, _attr->value, opt2_to_type<1, 1>());
		return os.str();
	} else {
		return "";
	}
...
bool xml_attribute::set_value(const char* rhs)
{
	...
	//--------------------------------------
	// add by zta , for decode xml first
	char * s = _attr->value;

	s = strconv_attribute(s,'\0',parse_default);
...
const char* xml_node::_value() const
{
...
{
	if(!empty() && _root->value) {

		std::ostringstream os;
		text_output_escaped(os, _root->value, opt2_to_type<1, 1>());
		return os.str();
	} else {
		return "";
	}
...
std::string xml_node::child_value() const
{
	if (!empty())
	for (xml_node_struct* i = _root->first_child; i; i = i->next_sibling)
	if ((static_cast<xml_node_type>(i->type) == node_pcdata || static_cast<xml_node_type>(i->type) == node_cdata) && i->value)
	{
		std::ostringstream os;
		text_output_escaped(os, i->value, opt2_to_type<1, 1>());
		return os.str();
	}
	return "";
}
...
std::string xml_node::child_value(const std::string& name) const
{
	return child(name.c_str()).child_value();
}

std::string xml_node::child_value_w(const std::string& name) const
{
	return child_w(name.c_str()).child_value();
}
...
bool xml_node::set_value(const char* rhs)
{
	switch (type())
	{
		...
		case node_comment:
		{
			...
			//-----------------------------------------
			// add by zta  for decode xml

			if(type() == node_pcdata) {
				char * s = _root->value;

				s = strconv_pcdata_set_value(s, parse_default);
...
xml_attribute xml_node::append_attribute(const char* name)
{
	...
	//-----------------------------------------------
	//add by zta for self request
	xml_attribute att = attribute(name);
	if(!att.empty()) return att;
	//end
	//-----------------------------------------------
	...
	switch (type())
	{
		...
		case node_element:
		{
			...
			for (xml_attribute a = first_attribute(); a; a = a.next_attribute())
			{
				...
				text_output_escaped(os, a._value(), opt2_to_type<1, 1>());
				else
				text_output_escaped(os, a._value(), opt2_to_type<1, 0>());
			...
			else if (_root->first_child == _root->last_child && first_child().type() == node_pcdata)
			{
				...
				if (flags & format_utf8)
				text_output_escaped(os, first_child()._value(), opt2_to_type<0, 1>());
				else
				text_output_escaped(os, first_child()._value(), opt2_to_type<0, 0>());
		...
		case node_pcdata:
		if (flags & format_utf8)
		text_output_escaped(os, _value(), opt2_to_type<0, 1>());
		else
		text_output_escaped(os, _value(), opt2_to_type<0, 0>());
		break;
...
void xml_node::print(std::string& os)
{
	if (empty()) return;
	...
	switch (type())
	{
		...
		case node_element:
		{
			...
			for (xml_attribute a = first_attribute(); a; a = a.next_attribute())
			{
				os.append(" ").append(a.name()).append("=\"");
		...
		case node_cdata:
		os.append("<![CDATA[").append(_value()).append("]]>");
		break;

		case node_comment:
		os.append("<!--").append(_value()).append("-->");
		break;

		case node_pi:
		os.append("<?").append(name());
		if (_value()[0]) os.append(" ").append(_value());
		os.append("?>");
		break;
