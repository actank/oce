...
NotifyBodySeq NotifyIndexNAdapter::GetNotifyBodyByNotifyIndex(const NotifyIndexSeq& indexseq, vector<Ice::Long>& not_in_db_nids){
	...
	try {
		...
		MCE_WARN("NotifyIndexNAdapter::GetNotifyBodyByNotifyIndex--> NotifyContentAdapter::getNotifyDict() error:" << e);
	}
	...
	for(NotifyContentSeq::iterator logit = contents.begin(); logit != contents.end(); ++logit){
		...
		MCE_DEBUG("NotifyIndexNAdapter::GetNotifyBodyByNotifyIndex-->id:" << body->id << " source:" << body->source << " owner:" << body->owner << " from :" << body->from << " type:" << body->type << " time:" << body->time <<  " props:" << os.str());
	}
	return bodyseq;
}
...
void NotifyIndexNAdapter::TestGet1(int userid, int begin, int limit, int type, int view, int bigtype){
	if(userid % 10 == 1){
		...
		limit = rand() % 5 + 1;
		//if(userid % 100 == 1){
			MCE_INFO("NotifyIndexNAdapter::TestGet1-->userid:" << userid << " begin:" << begin << " limit:" << limit << " bigtype:" << bigtype);
			for(int i = 0; i < 2; ++i){
				getManager(userid)->GetRMessageByBigtype(userid, begin, limit, bigtype);
				MyUtil::IntSeq types;
				types.push_back(14);
				types.push_back(15);
				getManager(userid)->GetRMessageByType(userid, types, begin, limit, bigtype);
				getManager(userid)->GetRMessageByTypeAll(userid, types, begin, limit);
				getManager(userid)->GetUnreadCountByBigtype(userid, bigtype);
				MyUtil::IntSeq bigtypes;
				bigtypes.push_back(1);
				bigtypes.push_back(2);
				bigtypes.push_back(3);
				getManager(userid)->GetUnreadCountByBigtypes(userid, bigtypes);
				getManager(userid)->GetUndealCountByBigtype(userid, bigtype);
				getManager(userid)->GetUndealCountByType(userid, bigtype);
				getManager(userid)->GetUndealCountByTypes(userid, types);
			}
		//}
	}
}
...
void NotifyIndexNAdapter::RemoveRMessageByIds(int uid, const MyUtil::LongSeq& notifyIds, int type, int bigtype){
	if(notifyIds.empty()){
		return;
	}
